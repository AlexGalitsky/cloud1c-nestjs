...
### Работа с базами 1С
1. Пользователь создаёт базу 1С через POST `/api/bases` с параметрами и опционально .dt файлом
2. Сервис создаёт запись в БД со статусом `processing`
3. При наличии файла запускается восстановление через команду:
   ```
   1cv8.exe CONFIG /S"<server>" /N"<user>" /P"<pass>" /RestoreIB "<dt>" /Out "<log>" /UC "<code>"
   ```
4. После завершения обновляется статус (`ready`/`error`) и лог
5. Пользователь может загрузить новые .dt файлы и применить их
